    ...
    Q.audio.play = function(s,options) {
      ...
      for (var i=0;i<Q.audio.channels.length;i++) {
        ...
        if (!Q.audio.channels[i]['loop'] && Q.audio.channels[i]['finished'] < now) {	
          ...
          // Change volume of sound
          if(options && options['volume']) {
            Q.audio.channels[i]['channel'].volume = options['volume'];
          }
  ...
  */
  ...
  Q.StageSelector = Q.Class.extend({
    ...
    init: function(stage,selector) {
      ...
      this.items = this.stage !== undefined ? this.stage.lists[this.selector] : this.emptyList;
      this.length = this.items !== undefined ? this.items.length : 0;
    },
